...
const UsersRolesScreen = ({onBack, adminToken}) => {
    ...
    const loadData = async () => {
      try {
        ...
        const [usersResponse, departmentsResponse] = await Promise.all([
          fetch('https://attendance-backend-11.onrender.com/api/users', {headers}),
          fetch('https://attendance-backend-11.onrender.com/api/departments', {headers}),
        ]);
  ...
  const saveUser = async form => {
    try {
      ...
      const response = await fetch(`https://attendance-backend-11.onrender.com/api/users${editingUser ? `/${editingUser.id}` : ''}`, {
  ...
  const deleteUser = user => {
    Alert.alert('Delete user', `Remove ${user.name}?`, [
      {text: 'Cancel', style: 'cancel'},
      {
        ...
        style: 'destructive',
        onPress: () => {
          fetch(`https://attendance-backend-11.onrender.com/api/users/${user.id}`, {method: 'DELETE', headers: {Authorization: `Bearer ${adminToken}`}})
            .then(async response => {
              const result = await response.json();
